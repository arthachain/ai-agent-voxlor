...
import { aiAgentHandlers } from "./handlers/ai_agent_handlers";
...
export function registerIpcHandlers() {
  ...
  Object.entries(aiAgentHandlers).forEach(([key, handler]) => {
    const channel = `ai-agent:${key}`;
    // Register with the IPC system
    // This would typically be done through the existing IPC registration system
    console.log(`Registered AI Agent handler: ${channel}`);
  });
}
