...
// @ts-ignore
import logo from "../../assets/voxlor-logo.jpg";
import { providerSettingsRoute } from "@/routes/settings/providers/$provider";
import { cn } from "@/lib/utils";
import { useDeepLink } from "@/contexts/DeepLinkContext";
import { useEffect, useState } from "react";
import { VoxlorProSuccessDialog } from "@/components/VoxlorProSuccessDialog";
...
export const TitleBar = () => {
  ...
  const showVoxlorProSuccessDialog = () => {
  ...
  useEffect(() => {
    const handleDeepLink = async () => {
      if (lastDeepLink?.type === "voxlor-pro-return") {
        await refreshSettings();
        showVoxlorProSuccessDialog();
      }
    };
    handleDeepLink();
  }, [lastDeepLink]);
  ...
  const isVoxlorPro = !!settings?.providerSettings?.auto?.apiKey?.value;
  const isVoxlorProEnabled = Boolean(settings?.enableVoxlorPro);
  ...
  return (
    <>
      <div className="@container z-11 w-full h-11 bg-(--sidebar) absolute top-0 left-0 app-region-drag flex items-center">
        ...
        <img src={logo} alt="Voxlor Logo" className="w-6 h-6 mr-0.5" />
        <Button
        ...
        {isVoxlorPro && <VoxlorProButton isVoxlorProEnabled={isVoxlorProEnabled} />}
...
export function VoxlorProButton({
  isVoxlorProEnabled,
}: {
  isVoxlorProEnabled: boolean;
}) {
  const { navigate } = useRouter();
  const { userBudget } = useUserBudgetInfo();
  return (
    <Button
      data-testid="title-bar-voxlor-pro-button"
      onClick={() => {
        navigate({
      ...
      className={cn(
        ...
        !isVoxlorProEnabled && "bg-zinc-600 dark:bg-zinc-600",
      )}
      size="sm"
    >
      {isVoxlorProEnabled ? "Pro" : "Pro (off)"}
      {userBudget && isVoxlorProEnabled && (
